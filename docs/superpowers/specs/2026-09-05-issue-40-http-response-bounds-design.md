...
The bounded bytes will be converted with UTF-8 lossy conversion, matching the existing text-oriented API while avoiding a panic when a byte limit splits a multibyte character. Small responses will produce the same body and `size_bytes` values as before.
...
| Small responses unchanged | Keep the request flow and response fields intact; only add a false flag |
